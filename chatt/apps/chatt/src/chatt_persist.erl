...
load_rooms() ->
    DdbConfig = get_dynamo_config(),
    {ok, Resp} = erlcloud_ddb:scan("ChattRooms", [], DdbConfig), % Pass DdbConfig
...
load_user_invites(User) ->
    ...
    {ok, Result} = erlcloud_ddb:query("ChattInvites", [{key_conditions, KeyCond}], DdbConfig), % Pass DdbConfig
